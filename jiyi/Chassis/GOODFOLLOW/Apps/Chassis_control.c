...
//		pf = pow(X_Move - Yaw*arm_sin_f32(theta),2) + pow(Y_Move + Yaw*arm_cos_f32(theta),2) ;
//		kf = sqrt(pf);
//		sn = arm_sin_f32(theta);
//		Target_velocity[2] = pf;
...
int gogogogogo = 0;
int shshshsh = 0;
float angel_init[] = {-131.07f,-102.11f, -78.3f,127.66f};
void Ship_calc(float X_Move,float Y_Move,float Yaw ,float *Angle) 
{
    ...
    if(X_Move == 0 && Y_Move == 0 && Yaw == 0)
    {
		...
		if(Robot.ChassisWorkMode == ChassisWorkMode_Square)
		{
						M6020s[0].Init_angle = (-45) + angel_init[0];
						M6020s[1].Init_angle = (+45) + angel_init[1];
						M6020s[2].Init_angle = (-45) + angel_init[2];
						M6020s[3].Init_angle = (+45) + angel_init[3];
		}
		else
		{
						M6020s[0].Init_angle = (-45) + angel_init[0];
    ...
    else
    {
		...
		{
			M6020s[0].Init_angle =  angel_init[0];
			M6020s[1].Init_angle =  angel_init[1];
			M6020s[2].Init_angle =  angel_init[2];
			M6020s[3].Init_angle =  angel_init[3];
		}
...
int MinOffset(int target, int value)
{
...
//    float angle =  Cloud_getYawAngleWithCenter() + Yaw_Spin_Error; //机械角度偏差
		float angle = MinOffset(M6020s_Yaw.realAngle,2063)/22.7527f;
...
//					}
      ...
      Limit(drv_tempcurrent,4);//功率限制里面有超电的功率限制，超电读取值为0->输出值为0，加上超电改我的宏定义 Power_online 1 ;
	    for(uint8_t i = 0 ; i < 4 ; i++)
      {
