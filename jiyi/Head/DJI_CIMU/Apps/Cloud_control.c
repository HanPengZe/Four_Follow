...
int change_online = 0;
int gogodeng = 0;
const float Cloud_Pitch_Min = -900 ;
 float Cloud_Pitch_Max = -1921;
uint16_t init_cnt = 0;
...
void Pit_limt_hpz()
{
	if(M6020s_Pitch.targetAngle<=3700)
	{
		M6020s_Pitch.targetAngle=3700;
	}
	else if(M6020s_Pitch.targetAngle>=4700)
	{
		M6020s_Pitch.targetAngle=4700;
	}
	else{}
...
//			Filter_IIRLPF(&VisionData.RawData.y, &M6020s_Pitch.rc_targetAngle, 0.2); //利用低通平滑一下视觉传来的数据
			...
			M6020s_Pitch.targetAngle =  22.7527f * DJI_C_IMU.pitch + M6020s_Pitch.rc_targetAngle *0.8f ;
			M6020s_Yaw.targetAngle =  22.7527f * DJI_C_IMU.total_yaw  + M6020s_Yaw.rc_targetAngle;
	...
	{		
		...
		M6020s_Pitch.targetAngle +=  VPitch *0.02f;//遥控的通道值反了去掉了负号
		M6020s_Yaw.targetAngle += - VOmega * 0.02f;
	}
...
//	Pit_AngleLimit(&M6020s_Pitch.targetAngle);
	...
	Motor_0x1FF_SendData(0,M6020s_Yaw.i_outCurrent,-M6020s_Pitch.i_outCurrent,0);
