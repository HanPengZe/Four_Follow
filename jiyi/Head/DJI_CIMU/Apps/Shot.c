...
//以下为用摇杆的小陀螺模式自动击打的代码
	...
	if(Robot.ControlSource != ControlSource_PC && onlineyaw == 1 && To_mode == 5)    
	{
			if(DR16.rc.ch4_DW  >= 620)
			{
				if(VisionData.RawData.whether_Fire == 1 && VisionData.RawData.mode == 1)
				{
				...
				else if(VisionData.RawData.whether_Fire == 0)
				{
					shot.Key_need = 0;
				}
			}
	...
	{
		if(((VisionData.RawData.whether_Fire == 1 && VisionData.RawData.mode == 1) || DR16_Fun.GetKeyMouseAction(MOUSE_Left, KeyAction_CLICK) || DR16_Fun.GetKeyMouseAction(MOUSE_Left, KeyAction_PRESS)) && Auto_Shot == 1)
		{
				if(Vision_Shot_Hold == 0)
				{
					shot.Key_need ++;
